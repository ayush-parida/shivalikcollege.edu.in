...
import bbaData from '../../../../data/bba.json';

interface BbaLayoutProps {
  children: React.ReactNode;
}

export default function BbaLayout({ children }: BbaLayoutProps) {
  const pathname = usePathname();
  const { hero, tabs } = bbaData;

  // Determine current tab from pathname
  const getCurrentTab = () => {
    if (pathname === '/courses/bba') return 'home';
      ...
      <div className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-gray-900 font-medium">BBA</span>
          </nav>
        </div>
      </div>
            ...
            <div className="border-b border-slate-200">
              <nav className="flex flex-wrap gap-2 -mb-px">
                {tabs.map((tab: { id: string; label: string }) => {
                  ...
                  const href = tab.id === 'home' ? '/courses/bba' : `/courses/bba/${tab.id}`;
