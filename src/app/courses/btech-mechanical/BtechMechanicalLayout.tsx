...
import btechMechanicalData from '../../../../data/btech-mechanical.json';

interface BtechMechanicalLayoutProps {
  children: React.ReactNode;
}

export default function BtechMechanicalLayout({ children }: BtechMechanicalLayoutProps) {
  const pathname = usePathname();
  const { hero, tabs } = btechMechanicalData;

  // Determine current tab from pathname
  const getCurrentTab = () => {
    if (pathname === '/courses/btech-mechanical') return 'home';
            ...
            <div className="border-b border-slate-200">
              <nav className="flex flex-wrap gap-2 -mb-px">
                {tabs.map((tab: { id: string; label: string }) => {
                  ...
                  const href = tab.id === 'home' ? '/courses/btech-mechanical' : `/courses/btech-mechanical/${tab.id}`;
