...
import bcaData from '../../../../data/bca.json';

interface BcaLayoutProps {
  children: React.ReactNode;
}

export default function BcaLayout({ children }: BcaLayoutProps) {
  const pathname = usePathname();
  const { hero, tabs } = bcaData;

  // Determine current tab from pathname
  const getCurrentTab = () => {
    if (pathname === '/courses/bca') return 'home';
      ...
      <div className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-gray-900 font-medium">BCA</span>
          </nav>
        </div>
      </div>
            ...
            <div className="border-b border-slate-200">
              <nav className="flex flex-wrap gap-2 -mb-px">
                {tabs.map((tab: { id: string; label: string }) => {
                  ...
                  const href = tab.id === 'home' ? '/courses/bca' : `/courses/bca/${tab.id}`;
