...
import btechEceData from '../../../../data/btech-ece.json';

interface BtechEceLayoutProps {
  children: React.ReactNode;
}

export default function BtechEceLayout({ children }: BtechEceLayoutProps) {
  const pathname = usePathname();
  const { hero, tabs } = btechEceData;

  // Determine current tab from pathname
  const getCurrentTab = () => {
    if (pathname === '/courses/btech-ece') return 'home';
            ...
            <div className="border-b border-slate-200">
              <nav className="flex flex-wrap gap-2 -mb-px">
                {tabs.map((tab: { id: string; label: string }) => {
                  ...
                  const href = tab.id === 'home' ? '/courses/btech-ece' : `/courses/btech-ece/${tab.id}`;
