...
import btechCivilData from '../../../../data/btech-civil.json';

interface BtechCivilLayoutProps {
  children: React.ReactNode;
}

export default function BtechCivilLayout({ children }: BtechCivilLayoutProps) {
  const pathname = usePathname();
  const { hero, tabs } = btechCivilData;

  // Determine current tab from pathname
  const getCurrentTab = () => {
    if (pathname === '/courses/btech-civil') return 'home';
            ...
            <div className="border-b border-slate-200">
              <nav className="flex flex-wrap gap-2 -mb-px">
                {tabs.map((tab: { id: string; label: string }) => {
                  ...
                  const href = tab.id === 'home' ? '/courses/btech-civil' : `/courses/btech-civil/${tab.id}`;
